...
class simpleConverter:
	def __init__(self):
		...
		self.RTS = np.array([math.cos(self.sensorAngle),-math.sin(self.sensorAngle),self.RPS[0][0]], [math.sin(self.sensorAngle),math.cos(self.sensorAngle),self.RPS[0][1]], [0,0,1])
		self.WPR = np.array([3, 2])
		self.WTR = np.array([math.cos(self.robotAngle),-math.sin(self.robotAngle),self.WPR[0][0]], [math.sin(self.robotAngle),-math.cos(self.robotAngle),self.WPR[0][1]], [0,0,1])
	...
	def callback(self, msg):
		...
		self.SP = np.matmul(self.RTS,self.sensorPos)
		self.ROB.x = self.SP[0]
		self.ROB.y = self.SP[1]
		self.robot.publish(self.ROB)
		self.WP = np.matmul(self.WTR,self.SP)
